...
SELECT 'id',
       ...
       'create_time'
UNION ALL
SELECT COALESCE(UE.id, ""),
       COALESCE(UE.user_id, ""),
       COALESCE(UE.session_id, ""),
       COALESCE(UE.page, ""),
       COALESCE(UE.actions, ""),
       COALESCE(UE.city, ""),
       COALESCE(UE.properties, ""),
       COALESCE(UE.other_details, ""),
       COALESCE(convert_tz(UE.create_time, '+00:00', '+05:30'), '')
...
SELECT COALESCE(id, ""),
       ...
       COALESCE(native_lang, ""),
       COALESCE(convert_tz(create_time, '+00:00', '+05:30'), "")
FROM `dcag-db`.`users`
...
SELECT COALESCE(id, ""),
       ...
       COALESCE(price, ""),
       COALESCE(convert_tz(create_time, '+00:00', '+05:30'), ""),
       COALESCE(convert_tz(last_updated_time, '+00:00', '+05:30'), ""),
       COALESCE(convert_tz(due_time, '+00:00', '+05:30'), ""),
       COALESCE(convert_tz(start_time, '+00:00', '+05:30'), ""),
       COALESCE(is_available, ""),
       COALESCE(task_category, "")
FROM `dcag-db`.`tasks`;
...
UNION ALL
SELECT COALESCE(UI.id, ""),
       COALESCE(UI.user_id, ""),
       COALESCE(UI.task_type, ""),
       COALESCE(UI.summary, ""),
       COALESCE(UI.description, ""),
       COALESCE(convert_tz(UI.create_time, '+00:00', '+05:30'), "")
...
SELECT 'id',
       ...
       'last_updated_time'
UNION ALL
SELECT COALESCE(UT.id, ""),
       COALESCE(UT.user_id, ""),
       COALESCE(UT.task_id, ""),
       COALESCE(UT.status, ""),
       COALESCE(UT.use_input_as_output, ""),
       COALESCE(UT.output, ""),
       COALESCE(UT.output_desc, ""),
       COALESCE(convert_tz(UT.start_time, '+00:00', '+05:30'), ""),
       COALESCE(convert_tz(UT.completion_time, '+00:00', '+05:30'), ""),
       COALESCE(convert_tz(UT.last_updated_time, '+00:00', '+05:30'), "")
...
UNION ALL
select COALESCE(UT.user_id, ''),
       COALESCE(T.task_type, ''),
       COALESCE(UT.status, ''),
       COALESCE(U.city_name, ''),
       COALESCE(convert_tz(UT.start_time, '+00:00', '+05:30'), ''),
...
SELECT t.task_type AS 'Task Type', COUNT(DISTINCT ut.user_id) AS 'Number of Users Completed'
FROM user_tasks ut
         JOIN
...
select ue.user_id,
       ...
       u.preferred_lang,
       u.native_lang,
       ue.properties as 'App Language', ue.create_time
from user_events ue
         join users u on ue.user_id = u.id
where ue.page = 'account_page'
  and ue.properties = 'hindi'
  and u.user_type = 'DRIVER'
order by ue.create_time;


-- Total earnings (price of completed task) by  task type

SELECT t.task_type AS 'Task Type', ROUND(SUM(t.price), 2) AS 'Total Earnings (INR)'
FROM user_tasks ut
         JOIN
     tasks t ON ut.task_id = t.id
         JOIN
